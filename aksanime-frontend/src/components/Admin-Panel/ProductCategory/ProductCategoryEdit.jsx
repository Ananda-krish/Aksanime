...
export default function ProductCategoryEdit(){
    ...
    useEffect(() => {
        const fetchCategory = async () => {
            try {
                const response = await fetch(`http://127.0.0.1:8000/api/productcategory/productcategoryshow/${id}`);
    ...
    const handleUpdate = async (event) => {
        ...
        try {
            const response = await fetch(`http://127.0.0.1:8000/api/productcategory/productcategoryupdate/${id}`, {
