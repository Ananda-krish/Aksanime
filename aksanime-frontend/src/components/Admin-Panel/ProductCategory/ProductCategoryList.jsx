...
const navigate= useNavigate();
    ...
    const handleEdit = (id) => {
       navigate(`/admin-dashboard/edit/productcategory/${id}`); // Navigate to edit page with category ID
    };

    useEffect(() => {
        const fetchCategories = async () => {
            try {
                const response = await fetch("http://127.0.0.1:8000/api/productcategory/productcategorylist");
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Are you sure you want to delete this category?')) {
            try {
                const response = await fetch(`http://localhost:8000/api/productcategory/productcategorydelete/${id}`, {
                    method: 'DELETE',
                    headers: {
    ...
    return (
        <div className="admin-category-table">
            ...
            <Link to="/admin-dashboard/create/productcategory" className="link-button create-category-btn">
                Create Category
            </Link>
